...
export const CreatePost = () => {
  ...
  const [post_categories, setPost_Categories] = useState([]);  // Initialize as empty array
  ...
  useEffect(() => {
    const getAllCategory = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/category/all-categories`);
        console.log(response.data);
        setPost_Categories(response.data);
      } catch (error) {
        ...
        <form onSubmit={createPost} className="form create-post__from">
          ...
          <select onChange={(e) => inputOnChange("category", e.target.value)} value={blog.category}>
            ...
            {post_categories.map(cat => (
              <option key={cat._id} value={cat.title._id}>{cat.title}</option>
            ))}
          </select>
